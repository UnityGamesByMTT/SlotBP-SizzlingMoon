...
public class DOTweenUIManager : MonoBehaviour
{
    ...
    internal void Jump(RectTransform uiElement, float jumpPower, int numJumps, float duration, Action Completed)
    {
        uiElement.DOJumpAnchorPos(uiElement.anchoredPosition, jumpPower, numJumps, duration).OnComplete(() => { Completed?.Invoke(); });
    }
